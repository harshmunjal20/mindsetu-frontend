...
import { Theme, CurrentUser, UserType } from '../types'; // Updated to CurrentUser, added UserType
...
interface NavbarProps {
  theme: Theme;
  toggleTheme: () => void;
  currentUser: CurrentUser | null; // Updated prop
  onLogout: () => void;
}
...
export const Navbar: React.FC<NavbarProps> = ({ theme, toggleTheme, currentUser, onLogout }) => {
  ...
  const currentNavLinks = useMemo(() => {
    ...
    let links = NAV_LINKS_LOGGED_IN;
    if (currentUser?.userType === UserType.Student) {
      links = links.filter(link => link.name !== 'Subscription');
    }
    if (currentUser?.userType === UserType.Admin) {
      links = links.filter(link => link.name !== 'Journal' && link.name !== 'Subscription'); // Hide Journal & Subscription for Admin
    }
    return links;
    ...
    <nav className={`sticky top-0 z-50 shadow-md ${navBgColor}`}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center">
            <div className="ml-10 flex items-baseline space-x-1">
              {currentNavLinks.map((link) => {
                ...
                const isActive = location.pathname === link.path || (link.path === "/dashboard" && location.pathname.startsWith("/dashboard")); // Highlight dashboard for related paths
                return (
                  <Link
                    key={link.name}
                    to={link.path}
            ...
            {/* Login/User Info/Logout Button for Desktop */}
            <div className="ml-4 flex items-center">
              {!isLoggedIn ? (
                ...
                <div className="flex items-center space-x-3">
                  <span className={`text-sm font-medium ${userTextColor}`}>Hello, {currentUser.firstName}!</span>
                  <button
                    onClick={onLogout}
          ...
          <div className="flex items-center"> {/* Theme toggle and mobile menu button container */}
            <button
              onClick={toggleTheme}
