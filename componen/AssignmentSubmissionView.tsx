...
import { 
  ...
  getSubmissionsForInstitute, // To count submissions for admin view
...
import { 
  ...
  ClockIcon,
  BellIcon,
  ExclamationTriangleIcon, // Changed from: AlertTriangleIcon as ExclamationTriangleIcon
  CheckBadgeIcon
} from './icons'; // Assuming these are added or available
...
export const AssignmentSubmissionView: React.FC<AssignmentSubmissionViewProps> = ({ currentUser }) => {
  const [assignments, setAssignments] = useState<Assignment[]>([]); // For Admin
  const [studentDisplayableAssignments, setStudentDisplayableAssignments] = useState<StudentDisplayableAssignment[]>([]); // For Student
  const [studentSubmissions, setStudentSubmissions] = useState<StudentAssignmentSubmission[]>([]); // For Student, to derive alerts
  const [alerts, setAlerts] = useState<AssignmentAlert[]>([]);
  
  // For Admin - Create Assignment Form
  const [newAssignmentTitle, setNewAssignmentTitle] = useState('');
  const [newAssignmentDueDate, setNewAssignmentDueDate] = useState('');

  // For Admin - To show submission counts per assignment
  ...
  const [isSubmittingForm, setIsSubmittingForm] = useState(false); // For admin create form
  const [isSubmittingStudentAction, setIsSubmittingStudentAction] = useState<string | null>(null); // assignment.id for student submit button
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      if (currentUser.userType === UserType.Admin) {
        ...
        } // Error handling for this can be added if critical for admin view
        ...
        if (assignResponse.success && subsResponse.success && assignResponse.data && subsResponse.data) {
          setStudentSubmissions(subsResponse.data);
  ...
  const handleCreateAssignment = async (e: React.FormEvent) => {
    ...
    if (response.success) {
      ...
      fetchData(); // Refresh assignment list
    } else {
  ...
  const handleStudentSubmitAssignment = async (assignmentId: string) => {
    ...
    setFormMessage(null); // Clear general messages
    const response = await addStudentSubmission(currentUser.id, assignmentId, currentUser.instituteName);
    if (response.success) {
        // Display specific message for this action, maybe near the button or as a toast in a real app
        // For now, we just refetch which will update status and alerts.
        fetchData(); 
    } else {
  ...
  if (currentUser.userType === UserType.Admin) {
    return (
      <div className="space-y-8">
        <Card title="Create New Assignment" icon={<PlusCircleIcon className="w-6 h-6" />} className="border-b-4 border-brand-primary dark:border-brand-primary-light">
