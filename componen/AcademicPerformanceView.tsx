...
import { CurrentUser, SubjectGrade, OverallPerformance, UserType, AdminInstituteAcademicStats, AdminAcademicInsights, User, Assignment, StudentAssignmentSubmission } from '../types';
import { LineChart, Line, BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer, Cell } from 'recharts';
import { generateAdminAcademicInsights } from '../services/geminiService';
import { getInstituteAssignmentStats } from '../services/assignmentService'; // Import new service method
...
export const AcademicPerformanceView: React.FC<AcademicPerformanceViewProps> = ({ currentUser }) => {
  const [instituteStats, setInstituteStats] = useState<AdminInstituteAcademicStats | null>(null);
  const [insights, setInsights] = useState<AdminAcademicInsights | null>(null);
  const [isLoadingData, setIsLoadingData] = useState<boolean>(false); // For overall data loading
    ...
    if (currentUser.userType === UserType.Admin) {
      const fetchAdminData = async () => {
        ...
        try {
          ...
          setErrorData(err.message || "An error occurred while fetching admin data.");
          setInsights({ 
      ...
      <div className="space-y-6">
        ...
        >
            <p className="text-sm text-slate-600 dark:text-slate-400">
                This dashboard provides an aggregated overview of student academic engagement patterns within your institute. AI-generated insights are based on available data.
            </p>
        </Card>
