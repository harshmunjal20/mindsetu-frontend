...
export const SignupPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (response.success && response.data) {
      const defaultSuccessMsg = formData.userType === UserType.Admin 
        ? 'Admin account created successfully! Redirecting to login...'
        : 'Account activated successfully! Redirecting to login...';
      setSuccessMessage((response as any).message || defaultSuccessMsg);
      setTimeout(() => {
        navigate('/login');
      }, 2500);
    } else {
  ...
  const pageTitle = formData.userType === UserType.Admin ? 'Register Your Institute' : 'Complete Your Student Registration';
  const pageDescription = formData.userType === UserType.Admin 
    ? `Register as an Administrator for your institute to manage students and access ${APP_NAME}.`
    : `Complete your registration for ${APP_NAME}. You must be pre-registered by your institute's administrator.`;
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            >
              ...
              ) : (formData.userType === UserType.Admin ? "Register" : "Complete Signup & Activate")}
            </button>
          </div>
        </form>
